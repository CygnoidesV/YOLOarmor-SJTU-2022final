...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        # Inference
        # pred_raw = model(img)[0].detach().cpu()
        if pt:
            ...
            if dnn:
                net.setInput(img)
                pred_raw = torch.tensor(net.forward())
            else:
                pred_raw = torch.tensor(session.run([session.get_outputs()[0].name], {session.get_inputs()[0].name: img}))
